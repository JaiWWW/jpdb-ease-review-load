...
(function() {
    ...
    try {GM_getValue; GM_setValue} catch {
        // If GM functions are unavailable, map to script-identified localStorage variables instead
        var GM_getValue = (name) => localStorage.getItem(`EaseReviewLoad-${name}`);
        var GM_setValue = (name, value) => localStorage.setItem(`EaseReviewLoad-${name}`, value);
    }

    let Activated = GM_getValue('Activated') === 'true';
    let InitialRawDueCount = Number(GM_getValue('InitialRawDueCount'));
        ...
        try { // To make sure we restore visibility in case something goes wrong
            ...
            if (navDCSpan) {
                ...
                switch (url.slice(-7)) { // last 7 characters
                    case '/review':
                        if (!GM_getValue('FinishedBlockHTML')) setUpFinishedPageTemplate();
                    case 'jpdb.io':
                    case 'o/learn':
                ...
                */
                ...
                if (!Activated && rawDueCount > ACTIVATION_THRESHOLD) {
                    ...
                    GM_setValue('InitialRawDueCount', rawDueCount);
                    ...
                    if (rawDueCount <= Math.max(0, InitialRawDueCount - RESTRICTED_MAX)) {
                        ...
                        if (rawDueCount > ACTIVATION_THRESHOLD) {
                            ...
                            GM_setValue('InitialRawDueCount', rawDueCount); // Don't need to update script copy of the variable as we won't use it again
    ...
    function setUpFinishedPageTemplate() {
        ...
        GM_setValue('FinishedBlockHTML', bodyClone.innerHTML);
        debugLog(`Finished setting up page body innerHTML`);
    }

    function showFinishedSessionPage(count) {
        const bodyHTML = GM_getValue('FinishedBlockHTML');
